    ...
    def post_produce2(self, produce_state: Fab_ProduceState, tracing: str = "") -> None:
        ...
        if False or (solid_plane.UnitNormal - z_axis).Length > 1.0e-8:
            ...
            if not cnc_path.exists():
                ...
                max_cnc_x = 0.0  # mypy requires that min/max_cnc_x/y be initialized.  Why?
                min_cnc_x = 0.0  # TODO: Add back `: float`.
                max_cnc_y = 0.0
                min_cnc_y = 0.0
                ...
                cnc_contact: Vector = Vector(0.0, 0.0, 0.0)  # TODO: Fix self.StartDepth)
                ...
                for cnc_circle in cnc_circles:
                    cnc_query.move_to(cnc_circle.Center)  # TODO: Assume +Z axis for now.
                    cnc_query.hole(diameter, depth)
                self.HolesCount = len(cnc_circles)

                # Write *assembly* out to a Step file:
                assembly: cq.Assembly = cq.Assembly(
                    cnc_query.WorkPlane, name=step_base_name,
...
class FabMount(object):
    ...
    * OrientTranslate (float):
    ...
    # FabMount.OrientEnd:
    @property
    def OrientEnd(self) -> Vector:
        """Return the FabMount Orient End point."""
        return self._OrientEnd + self._Copy  # pragma: no unit cover
    ...
    def to_json(self) -> Dict[str, Any]:
        ...
        orient_end: Vector = self._OrientEnd
        orient_angle: Vector = self._OrientAngle
        json_dict: Dict[str, Any] = {
            "Kind": "Mount",
            "Label": self.Name,
            "_Contact": [contact.x, contact.y, contact.z],  # TODO: fix
            "_Normal": [normal.x, normal.y, normal.z],
            "_OrientAngle": math.degrees(orient_angle),
            "_OrientEnd": [orient_end.x, orient_end.y, orient_end.z],
            "_OrientStart": [orient_start.x, orient_start.y, orient_start.z],
